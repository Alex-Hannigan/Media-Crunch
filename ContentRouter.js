...
import JapaneseNews from './JapaneseNews.js';
import AmericanNews from './AmericanNews.js';
import IrishNews from './IrishNews.js';
...
class ContentRouter extends Component {
  ...
  componentWillMount() {
    ...
    .then(data => {
      this.setState({japaneseNews: {articles: data.articles, loading: false, selectedSource: "全て"}});
    })
    .catch(error => {
      console.log(error);
    });
    ...
    .then(data => {
      this.setState({americanNews: {articles: data.articles, loading: false, selectedSource: "All"}});
    })
    .catch(error => {
      console.log(error);
    });
    ...
    .then(data => {
      this.setState({irishNews: {articles: data.articles, loading: false, selectedSource: "All"}});
    })
    .catch(error => {
      console.log(error);
    });
  ...
  handleInputChange(event) {
    ...
    const name = target.name;
      if (name === 'japaneseSourceSelection') {
        const stateCopy = this.state;
        stateCopy.japaneseNews.selectedSource = value;
        this.setState(stateCopy);
      }
      else if (name === 'americanSourceSelection') {
        const stateCopy = this.state;
        stateCopy.americanNews.selectedSource = value;
        this.setState(stateCopy);
      }
      else if (name === 'irishSourceSelection') {
        const stateCopy = this.state;
  ...
  render() {
    ...
    // Function which returns the JapaneseNews component with props
    const JapaneseNewsToRender = () => {
      return (
        <JapaneseNews state={this.state.japaneseNews}
          handleInputChange={this.handleInputChange}
        />
      )
    }

    // Function which returns the AmericanNews component with props
    const AmericanNewsToRender = () => {
      return (
        <AmericanNews state={this.state.americanNews}
          handleInputChange={this.handleInputChange}
        />
      )
    }

    // Function which returns the IrishNews component with props
    const IrishNewsToRender = () => {
      return (
        <IrishNews state={this.state.irishNews}
          handleInputChange={this.handleInputChange}
        />
      )
    }
